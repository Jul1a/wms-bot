...
def show_category_tree(request):
  ...
  if oprt and (set_layer != -1) and set_obj:
    ...
    if ((oprt == 'add') and (set_obj.author_id == USER or set_obj.pub == 1)):
    ...
    if ((oprt == 'del') and (set_obj.author_id == USER or set_obj.pub == 1)):
      LayerTree.objects.del_fromset(set_layer, cursor)
    
    # Operation of off hidden layers
    if ((oprt == 'hidd_off') and set_layer and (set_obj.author_id == USER or set_obj.pub == 1)):
      layers = []
      if set_layer == -1:
        ll = (0, )
      else:
    ...
    if ((oprt == 'hidd_on') and set_layer and (set_obj.author_id == USER or set_obj.pub == 1)):
      layers = []
      if set_layer == -1:
        ll = (0, )
      else:
    ...
    if ((oprt == 'add_style') and (set_obj.author_id == USER or set_obj.pub == 1)):
    ...
    if ((oprt == 'del_style') and (set_obj.author_id == USER or set_obj.pub == 1)):
      LayerTree.objects.delstyle(set_layer)
    
    # Operation off the layer of publicity
    #if ((oprt == 'pub_off') and (set_obj.author_id == USER or set_obj.pub == 1)):
    #  layers = []
    #  if set_layer == -1:
    #     ll = (0, )
    #  else:
    ...
    if set_obj:
      ...
      if (title_group and (set_obj.author_id == USER or set_obj.pub == 1)):
    ...
    if addset:
      ...
      slt_set = (LayerSet.objects.add_newset(sname, stitle, sabstr, skeywords, int(spub), USER)).id
    ...
    if editset and set_obj:
      ...
      if set_obj.author_id == USER:
        LayerSet.objects.edit_set(slt_set, sname, stitle, sabstr, skeywords, int(spub), USER)
      else:
  ...
  for c in LayerSet.objects.all():
    if (c.author_id == USER) or (c.pub == 1):
      BIG_CHOICES.append((c.id, "(%s(%s"%(c.title, c.name)))
  ...
  for c in SLD.objects.all():
    if (c.owner_id == USER or not c.owner_id):
      BIG_CHOICES.append((c.name+c.url, "%s(%s)"%(c.name, c.url)))
  ...
  if not slt_set:
    ...
    for c in LayerSet.objects.all():
      if (c.pub == 1) or (c.author_id == USER):
        selected_set = c
        break;
  ...
  if id_server:
    cursor.execute('''
                      ...
                      (
                        ...
                        WHERE tree.server_id = %s and (tree_servers.owner_id = %s or tree.pub = true) and tree_servers.id = tree.server_id
  ...
  if id_set:
    cursor.execute('''
                      ...
                      (
                        ...
                        SELECT tree.name, tree.id, tree.parent_id, tree.lset_id, tree.layer_id, tree.hidden, tree.sld_id, tree.name, tree.path 
                        FROM tree, tree_layers, tree_servers
                        WHERE tree.lset_id = %s AND 
                              (
                                (tree.layer_id != NULL and tree.layer_id = tree_layers.id AND tree_layers.server_id = tree_servers.id
                                and tree_servers.owner_id = %s or tree_layers.pub = true)
                              OR
                                tree.layer_id IS NULL
                              )
                        GROUP BY tree.name, tree.id, tree.parent_id, tree.lset_id, tree.layer_id, tree.hidden, tree.sld_id, tree.path
